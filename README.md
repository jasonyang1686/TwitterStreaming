...
- What would a production-ready solution entail that a POC wouldn't?
  ...
  Basically, A POC is far away from production-ready solution.

  

- What is the level of effort required to deliver each phase of the solution? What is your estimated timeline for delivery for a production-ready solution?

  Suppose that we've established the feasibility of the pipeline from POC test, we will start considering the possibility of actual business model and creating corresponding release schedule. First we need to consider the business requirements and what kind of software and hardware architecture can satisfy those requirements. For example, update the current bandwidth to avoid streaming limitation and add more computers to form clusters to boost the processing and storing progress. It may take less than a month to finish this phase.

  Besides the requirements, we also need to consider the problem that was not happened in the POC test, for instant, a scalability issue (for example, exponential growth of tweets and queries need to be processed), and what's the cost for solving those kinds of problems. Without experience, we will need to pay for the efforts of starting from scratch. One option is we can spend own labor costs and software/hardware costs to evaluate the problem. For example, we need to study what kind of hardware and software architecture is needed to complete the requirements and what's the performance. And we need to study as the demand grows, how the performance changes and where is the bottleneck, how to expand it in the future, etc.  The drawback is it may cost a lot for laboring to finish this evaluation and we still may face some certain opportunity cost that our evaluation is incorrect or inaccurate, leading to higher operating and expansion costs in the future. Another one is relying on the technical support from 3rd party vendors who had overall experience on dealing with the conflicts of business requirements and the scalability of such data stream pipeline.  For example, we can subscribe Spark business version support, which provides more reliable and stable service than community version one. In this case it costs us a lot for consulting. It depends on which option is selected; the estimated time may vary from one week to six month.

  Then we can starting develop each component of the production-ready solution based on the prototype from POC and requirements from Functional Requirements Document (FRD). I would suggest to choose agile methodology which use story points as units of measure for expressing an estimate of the level of effort for each phase. We can form agile teams into DEV, SIT, UAT and PROD.   DEV should consider failure handling and recovery mechanism and QA should consider the boundary condition outsides the project itself. For example, the network security protocol is different between DEV/QA/UAT and PROD environment, or stress test in PROD environment (PROD will run many other applications on his computer but DEV/QA/UAT may not). I would also suggest every story or task should be well documented, and DEV team should provide as detailed operation manual and training as possible, to let PROD team to operate the solution with the least communication cost.  In this phase the project will continue updating and releasing, so the timeline for each task could be referenced from agile story points (for example, jira).
